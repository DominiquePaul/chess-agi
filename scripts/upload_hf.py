...
def parse_args():
    ...
    parser.add_argument("--model-type", type=str, default="yolo", help="Type of model architecture")
    parser.add_argument(
        "--task",
        type=str,
        default=None,
...
def validate_args(args) -> bool:
    ...
    if "/" not in args.repo_name:
        ...
        print("💡 Example: 'myusername/chess-corner-detector'")
        return False
...
def upload_model(model_path: Path, args) -> bool:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print("📁 Model copied to temporary directory")
            ...
            if args.include_training_dir:
                ...
                if training_dir.exists():
                    ...
                    print("📂 Training directory copied")
...
def main():
    ...
    if success:
        ...
        if not args.dry_run:
            print("\n🎯 Next steps:")
            print("1. Test your uploaded model:")
            if args.model_task == "corner-detection":
                print(
                    f"   python src/chess_board_detection/test_model.py --model {args.repo_name} --image path/to/image.jpg"
                )
            elif args.model_task == "segmentation":
                print(
                    f"   python src/chess_board_detection/yolo/segmentation/test_segmentation.py --model {args.repo_name} --image path/to/image.jpg"
                )
            elif args.model_task == "detection":
                print(
                    f"   python src/chess_piece_detection/test_model.py --model {args.repo_name} --image path/to/image.jpg"
                )
            print("2. Update model card with performance metrics")
            print("3. Add example images to the repository")
    else:
        print("\n❌ Upload failed!")
