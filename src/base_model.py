...
from src.utils import get_best_torch_device, is_notebook, push_model_to_huggingface
...
class BaseYOLOModel:
    ...
    def __init__(
        ...
        """
        # Convert string paths to Path objects for base class compatibility
        ...
        if isinstance(model_path, str) and "/" in model_path and not Path(model_path).exists():
            ...
            self.model_path = self._load_from_huggingface(model_path)

        if self.model_path and self.model_path.exists():
            self.load_model()
        elif pretrained_checkpoint:
            print(f"Initialising with pretrained checkpoint: {pretrained_checkpoint}")
    ...
    def from_huggingface(
        ...
        """
        Load a model from a Hugging Face repository with cache detection.
        ...
        if hf_hub_download is None:
            raise ImportError(
                "huggingface_hub is required to load models from HF Hub. Please install it with: uv add huggingface_hub"
            )
        try:
            ...
            expected_cache_path = cache_dir / f"models--{repo_id.replace('/', '--')}" / "snapshots"
            is_cached = expected_cache_path.exists() and any(expected_cache_path.iterdir())

            # Download the model file from HF Hub
            model_path = hf_hub_download(repo_id=repo_id, filename=filename, cache_dir=cache_dir, token=token)
            ...
            # Set cache flag and update initialization message
            instance.loaded_from_cache = is_cached
            print(f"🚀 Model initialized on {instance.device} - loaded from {'cache' if is_cached else 'HF Hub'}")

            return instance

        except Exception as e:
            print(f"❌ Failed to load model from {repo_id}: {e}")
            raise
    ...
    def list_huggingface_files(cls, repo_id: str, token: str | None = None):
        ...
        if list_repo_files is None:
            raise ImportError(
                "huggingface_hub is required to list repository files. Please install it with: uv add huggingface_hub"
            )
    ...
    def _load_from_huggingface(self, model_name: str) -> Path:
        ...
        """
        try:
            # Check if model is already cached using base class helper
            cache_dir = "models/huggingface_cache"
            self.loaded_from_cache = self._check_huggingface_cache(model_name, cache_dir)

            # Download the model file
            # Models on HuggingFace Hub are uploaded as "model.pt"
            model_file = hf_hub_download(repo_id=model_name, filename="model.pt", cache_dir=cache_dir)

            return Path(model_file)
        except Exception as e:
            raise Exception(f"Failed to load model from HuggingFace Hub: {e}") from e

    def _check_huggingface_cache(self, repo_id: str, cache_dir: str | Path = "models/huggingface_cache") -> bool:
        """
        ...
        expected_cache_path = cache_dir / f"models--{repo_id.replace('/', '--')}" / "snapshots"
        return expected_cache_path.exists() and any(expected_cache_path.iterdir())
    ...
    def evaluate(self, data_path: Path, conf: float = 0.25, iou: float = 0.5):
        ...
        # Run validation
        results = self.model.val(data=data_path, conf=conf, iou=iou, device=self.device)
