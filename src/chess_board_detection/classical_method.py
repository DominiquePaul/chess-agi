...
def analyze_chess_board(input_image, model_path="models/yolo_chess_piece_detector/training_v2/weights/best.pt", target_size=512, conf=0.5):
    ...
    try:
        ...
        for i in range(rows - 1, -1, -1):
            for j in range(cols):
                ...
                squares_data_warped.append([
                    (x_center, y_center), bottom_right_sq, top_right_sq, top_left_sq, bottom_left_sq
                ])
        ...
        image_with_polygons, square_coordinates_data = process_square_coordinates_and_draw(processed_image_path, squares_data_original)
        ...
        for point in extreme_points:
            if point is not None:
                cv2.circle(extreme_points_image, (int(point[0]), int(point[1])), 15, (255, 255, 255), -1)
        ...
        original_squares_data = remap_coordinates(squares_data_original, offset_x, offset_y, scale)
        ...
        results = {
            ...
            'squares_processed': square_coordinates_data,
    ...
    if show_corners and results['corners_original']:
        ...
        for i, point in enumerate(corner_points):
            if point is not None:
                ...
                # Add labels
                labels = ['TL', 'TR', 'BL', 'BR']
                cv2.putText(vis_image, labels[i], (x-10, y-20), 
        ...
        for cell_id, coords in squares.items():
            if len(coords) >= 5:
                ...
                if cell_id <= 64:  # Only show for valid chess squares
                    ...
                    cv2.putText(vis_image, str(cell_id), (center_x-10, center_y+5), 
    ...
    if show_pieces and results['piece_detections'] and results['piece_detections'].boxes is not None:
        ...
        for i, box in enumerate(boxes):
            ...
            if confidences is not None and i < len(confidences):
                ...
                cv2.rectangle(vis_image, (orig_x1, orig_y1 - text_height - 5), 
                             (orig_x1 + text_width, orig_y1), color, -1)
                cv2.putText(vis_image, label, (orig_x1, orig_y1 - 2), 
...
def example_usage():
    ...
    try:
        ...
        print(f"Board corners (original coordinates):")
