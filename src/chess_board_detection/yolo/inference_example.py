...
def test_corner_detection(model_path: Path, image_path: Path):
    ...
    print(f"🎯 Testing Corner Detection")
    print("=" * 50)
        ...
        try:
            ...
            if coordinates:
                ...
                for j, coord in enumerate(coordinates):
                    print(f"     Corner {j+1}: ({coord['x']:.1f}, {coord['y']:.1f}) confidence: {coord['confidence']:.3f}")
...
def test_corner_ordering(model_path: Path, image_path: Path, conf: float = 0.25):
    ...
    for i, (name, corner) in enumerate(zip(corner_names, ordered_corners)):
    ...
    for i, corner in enumerate(ordered_corners):
        ax2.plot(corner['x'], corner['y'], 'ro', markersize=10, markerfacecolor='yellow', 
                markeredgecolor='red', markeredgewidth=2)
        ax2.text(corner['x'], corner['y'], str(i+1), ha='center', va='center', 
    ...
    print(f"📊 Ordering results saved to: output/corner_ordering_test.jpg")
    plt.show()
