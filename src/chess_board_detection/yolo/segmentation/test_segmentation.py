...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Maximum number of chessboard segments to detect (default: 1)",
    )
    parser.add_argument(
        "--extract-corners",
        action="store_true",
        help="Extract and display corner coordinates",
    )
    parser.add_argument(
        "--corner-method",
        type=str,
...
def test_single_image(model, image_path, args):
    ...
    polygon_info, is_valid = model.get_polygon_coordinates(
        image_path, conf=args.conf, iou=args.iou, max_segments=args.max_segments
    )

    if is_valid and polygon_info:
        print("✅ Successfully detected chessboard polygon(s)")
            ...
            for i, segment in enumerate(polygon_info.get("segments", [])):
                ...
                if i == 0 and "coordinates" in segment:
                    ...
                    if coords:
                        ...
                        print(
                            f"     Coordinate ranges: X=[{min(x_coords):.1f}, {max(x_coords):.1f}], Y=[{min(y_coords):.1f}, {max(y_coords):.1f}]"
                        )
        else:
        ...
        print("❌ No valid chessboard polygon detected")
    ...
    if args.extract_corners and is_valid and polygon_info:
        ...
        try:
            ...
            if corners and len(corners) == 4:
                print("✅ Successfully extracted 4 corners:")
                corner_names = ["Top-Left", "Top-Right", "Bottom-Right", "Bottom-Left"]
                for corner, name in zip(corners, corner_names, strict=False):
    ...
    if args.json_output:
        ...
        if corners_info and corners_info["corners"]:
            ...
            for corner, name, color in zip(corners, corner_names, corner_colors, strict=False):
                ...
                ax.text(
                    ...
                    fontsize=12,
                    weight="bold",
                    bbox={"facecolor": color, "alpha": 0.8, "edgecolor": "white", "linewidth": 2},
                )
            ...
            if corners_info.get("method") == "extended" and "debug_info" in corners_info:
                ...
                print("🔍 Adding extended method visualization...")
                    ...
                    if f"{side_key}_method" in debug_info:
                        ...
                        if len(corners_used) == 2 and method == "contour_with_exclusion":
                            ...
                            ax.text(
                                ...
                                weight="bold",
                                ha="center",
                                bbox={"facecolor": segment_color, "alpha": 0.8, "edgecolor": "white"},
                            )
            ...
            ax.text(
                15,
                100,
                f"Corner Method: {method_used}",
                color="white",
                fontsize=12,
                weight="bold",
                bbox={"facecolor": "purple", "alpha": 0.8, "edgecolor": "white", "linewidth": 2},
            )
