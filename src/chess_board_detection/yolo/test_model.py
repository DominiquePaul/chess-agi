...
Usage:
    ...
    python src/chess_board_detection/yolo/test_model.py --model models/chess_board_detection/corner_detection_training/weights/best.pt --image path/to/image.jpg

    # Test on multiple images in a directory
    python src/chess_board_detection/yolo/test_model.py --model models/chess_board_detection/corner_detection_training/weights/best.pt --input-dir path/to/images/

    # Test and get corner coordinates as JSON
    python src/chess_board_detection/yolo/test_model.py --model models/chess_board_detection/corner_detection_training/weights/best.pt --image image.jpg --json-output

    # Run from project root as module
    python -m src.chess_board_detection.yolo.test_model --help

Examples:
    # Basic test with visualization
    python src/chess_board_detection/yolo/test_model.py --model models/chess_board_detection/corner_detection_training/weights/best.pt --image examples/chessboard.jpg

    # Batch test all images in a folder
    python src/chess_board_detection/yolo/test_model.py --model models/chess_board_detection/corner_detection_training/weights/best.pt --input-dir examples/test_images/ --output-dir results/

    # Get coordinates for integration
    python src/chess_board_detection/yolo/test_model.py --model models/chess_board_detection/corner_detection_training/weights/best.pt --image board.jpg --json-output --save-coords coordinates.json
"""
...
def test_single_image(model, image_path: Path, args) -> dict | None:
    """Test model on a single image and return results."""
    try:
        if args.verbose:
            print(f"🔍 Processing image: {image_path}")

        # Get corner coordinates (fix: unpack the tuple correctly)
        corners, is_valid = model.get_corner_coordinates(str(image_path), conf=args.conf_threshold)
        ...
        if args.verbose:
            if corners:
                ...
                for i, corner in enumerate(corners):
                    ...
                    print(f"   Corner {i + 1}: ({x:.1f}, {y:.1f}) confidence: {conf:.2f}")
            else:
        ...
        if not args.no_save:
            ...
            try:
                ...
                model.plot_eval(str(image_path), ax=ax, conf=args.conf_threshold)
