...
try:
    ...
    for conf in confidence_levels:
        ...
        if v1_count > 0:
            ...
            improvement = "N/A" if v2_count == 0 else f"+∞"
    ...
    try:
        ...
        fig.suptitle('Model Comparison: v1 vs v2 at Different Confidence Levels', fontsize=16, fontweight='bold')
        ...
        for i, result in enumerate(comparison_results):
            ...
            ax_v1.set_title(f"v1 Model (conf={conf}): {v1_count} detections", fontsize=12, fontweight='bold')
            
            # Bottom row: v2 model results
            ax_v2 = axes[1, i]
            model_v2.plot_eval(single_eval_img_path, ax=ax_v2, conf=conf)
            ax_v2.set_title(f"v2 Model (conf={conf}): {v2_count} detections", fontsize=12, fontweight='bold')
    ...
    try:
        ...
        bars1 = ax.bar([i - width/2 for i in x], v1_counts, width, label='v1 Model (39% mAP50)', color='lightcoral', alpha=0.8)
        bars2 = ax.bar([i + width/2 for i in x], v2_counts, width, label='v2 Model (71% mAP50)', color='lightblue', alpha=0.8)
