...
AVAILABLE_DATASETS = {
    ...
    "all": [
        "dopaul/chess-pieces-dominique",
        "dopaul/chess-pieces-roboflow",
...
Examples:
  ...
  %(prog)s --no-convert                  # Download in HF format (no YOLOv8 conversion)
    ...
    parser.add_argument("--custom-name", help="Custom local name for custom repository")
    ...
    for hf_repo, local_name in datasets_to_download:
        success = download_dataset(hf_repo, local_name, args.output_dir, convert_to_yolo=not args.no_convert)
        if success:
            success_count += 1
        print()
