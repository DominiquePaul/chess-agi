...
class ChessDatasetUploader:
        ...
        for split in ["train", "valid", "test"]:
            ...
            for dataset_name, dataset in all_datasets.items():
                if split in dataset:
                    ...
                    # Create a closure to capture the dataset_name properly
                    def add_source(x, name=dataset_name):
                        return {**x, "source_dataset": name}

                    dataset_with_source = dataset[split].map(add_source)
    ...
    def _upload_additional_files(self, dataset_path: Path, repo_id: str, description: str | None = None):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Upload files
            upload_folder(folder_path=temp_path, repo_id=repo_id, repo_type="dataset")
