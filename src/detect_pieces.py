...
chess_piece_dataset_path = kagglehub.dataset_download(
...
if not os.path.exists("runs/detect/train"):
    ...
    model.to(device)
    yaml_path = os.path.join(chess_piece_dataset_path, "Chess_pieces", "data.yaml.yaml")
    train_path = os.path.join(chess_piece_dataset_path, "Chess_pieces", "train", "images")
    valid_path = os.path.join(chess_piece_dataset_path, "Chess_pieces", "valid", "images")
    results = model.train(
        data=yaml_path,
        epochs=15,
        batch=32,
...
data_dir = os.path.join(chess_piece_dataset_path, "Chess_pieces", "valid", "images")
...
results = model.predict(chess_img_path, conf=0.1, save=True)[0]
